...
def _view_():
    ...
    singupBTN = ft.TextButton("Si no tienes cuenta, registrate aqui", on_click=lambda e: e.page.go('/singup'))
    user_name = TextField(label='Username', value='', autofocus=True)
    password = TextField(label='Password', value='')
    loginBTN = ft.TextButton("Login", on_click=lambda e: login_handler(e, user_name.value, password.value))
